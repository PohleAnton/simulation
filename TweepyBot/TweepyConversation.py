...
access_token_secret_elon = os.getenv("ACCESS_TOKEN_SECRET_ELON")

api_key_roger = os.getenv("API_KEY_ROGER")
api_secret_roger = os.getenv("API_SECRET_ROGER")
access_token_roger = os.getenv("ACCESS_TOKEN_ROGER")
access_token_secret_roger = os.getenv("ACCESS_TOKEN_SECRET_ROGER")

auth_elon = tweepy.OAuthHandler(api_key_elon, api_secret_elon)
auth_elon.set_access_token(access_token_elon, access_token_secret_elon)
api_elon = tweepy.API(auth_elon)

auth_roger = tweepy.OAuthHandler(api_key_roger, api_secret_roger)
auth_roger.set_access_token(access_token_roger, access_token_secret_roger)
api_roger = tweepy.API(auth_roger)


# Funktion zum Aufteilen einer Konversation in Tweet-fähige Segmente von maximal 280 Zeichen.
def segment_conversation(conversation):
    max_length = 280
...
# Funktion zum Senden von Tweets aus einer Konversation, abwechselnd zwischen zwei Charakteren.
def send_tweets(api_elon, api_roger, conversation):
    segments = segment_conversation(conversation)
    for i, segment in enumerate(segments):
        try:
            if i % 2 == 0:
                # Elon tweetet
                api_elon.update_status(segment)
            else:
                # Roger tweetet
                api_roger.update_status(segment)
            time.sleep(10)  # Pausieren, um die Rate Limits von Twitter einzuhalten
        except tweepy.TweepyException as e:
            print(f"Fehler beim Senden des Tweets: {e}")
...
if loop_counter < 4:
    ...
    # ... Nachdem alle Argumente und Gegenargumente generiert wurden, wird getwittert
    send_tweets(api_elon, api_roger, conversation)
